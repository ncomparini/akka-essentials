...
class TimedAssertionsSpec extends TestKit(
  ...
  "A worker actor" should {
    ...
    "reply with the meaning of life in a timely manner" in {
      within(500 millis, 1 second) { // if we change the sleep to 300, this test it fails
        workerActor ! "work"
    ...
    "reply to a test probe in a timely manner" in {
      within(1 second) {
        ...
        probe.expectMsg(WorkResult(42)) // timeout of 0.3 seconds defined in config file!!
      }
    }
